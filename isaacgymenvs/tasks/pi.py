...
class Pi(VecTask):
    ...
    def __init__(self, cfg, sim_device, graphics_device_id, headless):
        ...
        #commands
        x_vel = torch.full((self.num_envs,),1.0,device='cuda:0')
    ...
    def _create_trimesh(self):
        ...
        heightfield[0:1400, :] = random_uniform_terrain(new_sub_terrain(), min_height=-0.06, max_height=0.06, step=0.05, downsampled_scale=0.15).height_field_raw
        ...
        def new_sub_terrain(): return SubTerrain(width=5, length=num_cols, vertical_scale=vertical_scale, horizontal_scale=horizontal_scale)
        ...
        heightfield[20:25, :] = stairs_terrain(new_sub_terrain(), step_width=0.25, step_height=0.3).height_field_raw
        heightfield[0:20, :] = sloped_terrain(slope_sub_terrain(), slope=0.001).height_field_raw
        heightfield[20:25, :] = stairs_terrain(new_sub_terrain(), step_width=0.25, step_height=0.3).height_field_raw
        heightfield[0:20, :] = sloped_terrain(slope_sub_terrain(), slope=0.001).height_field_raw
        heightfield[20:25, :] = stairs_terrain(new_sub_terrain(), step_width=0.25, step_height=0.3).height_field_raw
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        asset_file = "urdf/pi_description/pi.urdf"
        ...
        for i in range(self.num_envs):
            # create env instance
            env_ptr = self.gym.create_env(self.sim, env_lower, env_upper, 16)
            self.env_origins[i] = self.terrain_origins[self.terrain_levels[i], self.terrain_types[i]]
            pos = self.env_origins[i].clone()
            #pos[:2] += torch_rand_float(-1., 1., (2,1), device=self.device).squeeze(1)
            start_pose.p = gymapi.Vec3(*pos)
            
            anymal_handle = self.gym.create_actor(env_ptr, anymal_asset, start_pose, "pi", i, 1, 0)
    ...
    def pre_physics_step(self, actions):
        ...
        x_1 = torch.mul(self.actions[:,0],0.1)
        y_1 = torch.add(torch.mul(self.actions[:,1],0.02),0.005)
        z_1 = torch.add(torch.mul(self.actions[:,2],0.1),0.2)
        x_2 = torch.mul(self.actions[:,3],0.1)
        y_2 = torch.add(torch.mul(self.actions[:,4],0.02),0.005)
        z_2 = torch.add(torch.mul(self.actions[:,5],0.1),0.2)
        x_3 = torch.mul(self.actions[:,6],0.1)
        y_3 = torch.add(torch.mul(self.actions[:,7],0.02),0.005)
        z_3 = torch.add(torch.mul(self.actions[:,8],0.1),0.2)
        x_4 = torch.mul(self.actions[:,9],0.1)
        y_4 = torch.add(torch.mul(self.actions[:,10],0.02),0.005)
        z_4 = torch.add(torch.mul(self.actions[:,11],0.1),0.2)
    ...
    def post_physics_step(self):
        ...
        xyz = fk(self.dof_pos[:,0],self.dof_pos[:,1],self.dof_pos[:,2])
    ...
    def compute_reward(self):
        ...
        lin_y_vel_error = torch.square(torch.sub(self.commands[:,1] , self.base_lin_vel[:,1]))
        ang_vel_error = torch.square(torch.sub(self.commands[:, 2] , self.base_ang_vel[:, 2]))
        rew_forward = torch.mul((self.base_lin_vel[:,0]>0.15),(torch.abs(self.euler[:,2])<0.3))
        rew_lin_x_vel = torch.exp(-lin_x_vel_error/0.25)
        rew_lin_y_vel = torch.mul(torch.exp(-lin_y_vel_error/0.25),0.1)
        rew_ang = torch.mul(torch.exp(-ang_vel_error/0.25) , 0.2)
        ...
        rew_airTime = torch.mul(torch.sum((self.feet_air_time - 0.5) * first_contact, dim=1), 0.1)
        rew_contact = torch.mul(torch.add(torch.abs(contact[:,0]-contact[:,3]),torch.abs(contact[:,1]-contact[:,2])) ,-0.17)
        rew_dif_1 = torch.mul(torch.add(torch.abs(contact[:,0]-contact[:,1]),torch.abs(contact[:,2]-contact[:,3])) ,0.2)
        rew_dif_2 = torch.mul(torch.add(torch.abs(contact[:,0]-contact[:,2]),torch.abs(contact[:,1]-contact[:,3])) ,0.2)
        ...
        rew_base = torch.mul(torch.norm(self.euler[:,:2]),-0.05)
        ...
        total_reward = rew_forward + rew_airTime + rew_contact + rew_dif_1 + rew_dif_2 + rew_dof_pos
        ...
        # height_judge
        height_reset = torch.where(self.root_states[:,2]<0.238, self.one, self.zero).int()
...
def ik(x,y,z,num_envs):
    ...
    a=torch.full((num_envs,),0.006,device='cuda:0')
    ...
    n=torch.div((torch.sub(torch.pow(s,2),0.049925)),0.29)
    b=torch.full((num_envs,),0.1699999,device='cuda:0')
    c=torch.full((num_envs,),-0.169999,device='cuda:0')
    n=torch.where(n>=0.17,b,n)
    n=torch.where(n<=-0.17,c,n)
    beta=torch.mul(torch.acos(torch.div(n,0.17)),-1)
    alpha=torch.sub(torch.acos(torch.div(torch.add(0.145,n),s)),torch.atan(torch.div(x,l)))
...
def fk(gamma,alpha,beta):
    ...
    x=-torch.add(0.145*torch.sin(alpha),0.17*torch.cos(theta))
    y=h*torch.cos(gamma)+torch.mul(torch.add(0.145*torch.cos(alpha),0.17*torch.sin(theta)),torch.sin(gamma))
    z=h*torch.sin(gamma)+torch.mul(torch.add(0.145*torch.cos(alpha),0.17*torch.sin(theta)),torch.cos(gamma))
